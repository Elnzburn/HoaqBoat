...
def instantRestart(fro, chan, message):
	glob.streams.broadcast("main", serverPackets.notification("We are restarting Hoaqbot, Be right back!"))
...
def faq(fro, chan, message):
	...
	messages = {
		"rules": "Please make sure to check [https://osu.lumilous.pw/doc/rules].",
...
def restartShutdown(restart):
	...
	msg = "We are performing some maintenance. Hoaqboat will {} in 5 seconds. Thank you for your patience.".format("restart" if restart else "shutdown")
...
def systemReload(fro, chan, message):
	...
	return "Hoaqboat settings reloaded!"
...
def systemMaintenance(fro, chan, message):
	...
	if maintenance:
		...
		glob.streams.broadcast("main", serverPackets.notification("osu!lumilous server is in maintenance mode. Please try to login again later."))
...
def getBeatmapRequest(fro, chan, message): # Grab a random beatmap request. TODO: Add gamemode handling to this and !request
	...
	if request is not None:
		...
		return "[https://osu.lumilous.pw/u/{userID} {username}] nominated beatmap: [https://osu.ppy.sh/b/{beatmapID} {songName}] for status change. {BeatmapLink}The request has been deleted, so please decide it's status.".format(userID=request['userid'], username=username, beatmapID=request['bid'], songName=mapData['song_name'], BeatmapLink='[https://osu.lumilous.pw/b/{} Beatmap Link]. '.format(request['bid']))
	else:
...
def mm00(fro, chan, message):
	random.seed()
	return random.choice(["meme", "6969696969"])
...
def editMap(fro, chan, message): # Using Atoka's editMap with Aoba's edit
	...
	if chan.startswith('#') and chan != '#admin' and not privileges & 8388608:
		return "Map ranking is not permitted in regular channels, please do so in PMs with Kukuri (or #admin if administrator)."
	...
	if privileges & 256:
		...
		if glob.conf.config["discord"]["enable"]:
			...
			webhook = aobaHelper.Webhook(glob.conf.config["discord"]["ranked"], color=0xadd8e6, footer="This beatmap was {} on osu!lumilous".format(status))
			webhook.set_author(name=name, icon='https://a.lumilous.pw/{}'.format(str(userID)), url="https://osu.lumilous.pw/u/{}".format(str(userID)))
...
commands = [
	{
		...
		"trigger": "!help",
		"response": "Click (here)[https://osu.lumilous.pw/index.php?p=16&id=4] for full command list"
	}, {
		"trigger": "!ppboard",
