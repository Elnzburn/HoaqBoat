		...
		if glob.conf.extra["mode"]["anticheat"]:
			...
			if tornadoRequest.request.headers.get("ainu") == "happy":
				...
				if userUtils.isRestricted(userID):
					...
					webhook = aobaHelper.Webhook(glob.conf.config["discord"]["anticheat"],color=0xadd8e6,footer="Man... this is worst player.")
			...
			# hqOsu
				...
				if userUtils.isRestricted(userID):
					responseToken.enqueue(serverPackets.notification("Trying to use hqOsu in here? Go to play https://cookiezi.pw or others cheat server >w<"))
					#if glob.conf.config["discord"]["enable"] == True:
					webhook = aobaHelper.Webhook(glob.conf.config["discord"]["anticheat"],color=0xadd8e6,footer="Man... this is worst player.")
					webhook.set_title(title="Catched some cheater Account ID {}".format(userID))
